...
def consecutive(NEXT, all_labs, times):
    consec = []
    i = times.index(str(NEXT))
    for lab in all_labs:
        if lab[0][NEXT] == 'FREE':
            consec.append(lab)
    n_consec = []
    while i < len(times) - 1:
        n_consec = []
        for lab in consec:
            if lab[0][times[i]] == 'FREE':
                n_consec.append(lab)
        if len(n_consec) == 0:
            return consec, NEXT, times[i]
        consec = n_consec
        i += 1 
    return n_consec, NEXT, times[i]

def main():
    WEEK, DAY = DW() 
    labs = ['LG25', 'LG26', 'L101', 'L114', 'L125', 'L128']
    times = ['08:00','08:30','09:00','09:30','10:00','10:30','11:00','11:30','12:00','12:30','13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30','17:00', '17:30']
    NEXT = nexttime()
    all_labs = []
    with open('freelabs.txt', 'w') as f:
        f.write(NEXT+'\n')
        for lab in labs:
            d = check_lab(WEEK, DAY, lab)
            all_labs.append((d, lab))
            if d[NEXT] == 'FREE':
                f.write(lab+'\n')
    with open('conseclabs.txt', 'w') as f:
        consec, start, end = consecutive(NEXT, all_labs, times)
        f.write(start+'\n')
        f.write(end+'\n')
        for lab in consec:
            f.write(lab[1]+'\n')
